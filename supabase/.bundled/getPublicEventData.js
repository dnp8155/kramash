...
// supabase/functions/getPublicEventData/index.ts
Deno.serve(withCors(async (req) => {
  try {
    ...
    const { data: transactions } = await supabaseAdmin.from("financial_transactions").select("*").eq("workspace_id", event.workspace_id).eq("event_id", event.id).eq("transaction_type", "CLIENT_RECEIPT").eq("status", "ACTIVE").order("transaction_date", { ascending: false }).limit(200);
    const { data: assignments } = await supabaseAdmin.from("event_team_assignments").select("*").eq("workspace_id", event.workspace_id).eq("event_id", event.id).eq("assignment_status", "assigned").order("created_at", { ascending: true }).limit(100);
    const { data: members } = await supabaseAdmin.from("team_members").select("*").eq("workspace_id", event.workspace_id).eq("status", "active").order("name", { ascending: true }).limit(200);
    const { data: quotations } = await supabaseAdmin.from("quotations").select("*").eq("workspace_id", event.workspace_id).eq("event_id", event.id).in("status", ["finalized", "accepted"]).order("created_at", { ascending: false }).limit(10);
    const membersById = {};
    (members || []).forEach((m) => {
      membersById[m.id] = m;
    });
