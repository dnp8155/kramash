...
// supabase/functions/verifyTeamPortalAccess/index.ts
Deno.serve(withCors(async (req) => {
  try {
    const body = await req.json().catch(() => ({}));
    const { token, password } = body;
    if (!token || !password) return Response.json({ error: "Token and password are required" }, { status: 400 });
    const { data: teamMembers } = await supabaseAdmin.from("team_members").select("*").eq("portal_access_token", token).eq("portal_access_enabled", true).order("created_at", { ascending: false }).limit(5);
