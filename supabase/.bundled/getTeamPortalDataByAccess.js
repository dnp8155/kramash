...
// supabase/functions/getTeamPortalDataByAccess/index.ts
Deno.serve(withCors(async (req) => {
  try {
    ...
    const { data: teamMembers } = await supabaseAdmin.from("team_members").select("*").eq("id", team_member_id).eq("portal_access_token", session_token).eq("portal_access_enabled", true).order("created_at", { ascending: false }).limit(5);
