...
async function verifyStripeSignature(rawBody, signatureHeader, webhookSecret, toleranceSeconds = 300) {
  if (!signatureHeader || !webhookSecret) {
    throw new Error("Missing Stripe signature or webhook secret");
  }
  const parts = {};
  for (const item of signatureHeader.split(",")) {
    const [key2, ...rest] = item.split("=");
    parts[key2] = rest.join("=");
  }
  if (!parts.t || !parts.v1) {
...
// supabase/functions/handleStripeWebhook/index.ts
