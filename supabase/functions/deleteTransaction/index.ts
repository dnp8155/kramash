// deleteTransaction — Hard-delete + reconcile invoice/milestone.
...
Deno.serve(async (req) => {
  try {
    ...
    await supabaseAdmin.from("financial_transactions").delete().eq("id", transaction_id);
    const reconciled = await reconcileAfterTransactionChange(workspace_id, invoiceId, milestoneId);

    return Response.json({
      success: true, transaction_id, deleted: true,
