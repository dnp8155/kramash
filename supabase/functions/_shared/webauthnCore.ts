// WebAuthn core — challenge generation, CBOR decode, assertion verification.
// Adapted from base44/shared/webauthnCore.ts — uses Deno.env instead of base44:runtime secrets.
...
export function decodeCbor(data: Uint8Array, startOffset: number): { value: any; offset: number } {
...
// ===== Challenge token (stateless, HMAC-signed with APP_SECRET) =====

export async function createChallengeToken(challenge: Uint8Array, userId: string): Promise<string> {
  const payload = {
...
export async function verifyChallengeToken(token: string): Promise<any> {
...
function coseToJwk(coseKey: Record<string, any>): any {
  const kty = coseKey["1"];
  const crv = coseKey["-1"];
  const x = coseKey["-2"];
  const y = coseKey["-3"];
...
export function parseAuthData(authData: Uint8Array): any {
  ...
  const result: any = { rpIdHash, flags, counter, credentialId: null, credentialPublicKeyJwk: null };
...
export async function verifyAssertion(params: {
  ...
  let rpIdMatch = true;
  for (let i = 0; i < 32; i++) {
    if (expectedRpIdHash[i] !== authDataRpIdHash[i]) { rpIdMatch = false; break; }
  }
