...
> **Purpose:** This document is the single source of truth for migrating KRAMAS from Base44 to Firebase. It lists every pending setup item, every database collection with every field, every Cloud Function, and every configuration step. Codex/AI should read this document end-to-end and build the Firebase backend directly.
...
8. [Security Rules (Firestore & Storage)](#8-security-rules-firestore--storage)
9. [Data Migration Steps](#9-data-migration-steps)
...
- [ ] **Enable Firebase Storage** — For file uploads (quotations, signatures, workspace logos)
...
## 8. Security Rules (Firestore & Storage)

### 8.1 Firestore Rules Pattern
...
### 9.3 Migration Order (dependency-safe)
...
| `src/lib/firebaseConfig.js` | Update with real Firebase config |
...
□ 9. Deploy Firestore security rules (see Section 8)
...
□ 13. Update src/lib/firebaseConfig.js with real config + vapidKey
□ 14. Create src/lib/firebaseMessaging.js
□ 15. Create public/firebase-messaging-sw.js
□ 16. Update AuthContext to request notification permission
□ 17. Migrate existing data from Base44 (see Section 9)
□ 18. Test: auth, CRUD, push notifications, payments, AI agent
□ 19. Set up Razorpay/Stripe keys
□ 20. Set up Gemini API key for agentChat
```
...
**Target:** Firebase full migration