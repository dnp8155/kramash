...
export default function QuotationEditor() {
            ...
            if (!Array.isArray(estimateItems) || !estimateItems.length) {
              try {
                ...
                (serviceAsgns || []).forEach((a) => {
                  const rate = Number(a.agreed_rate) || 0;
                  imported.push({
                    item_type: "service", reference_id: a.service_id,
                    ...
                    unit_rate: rate, rate_type: a.rate_type || "Fixed",
                    days: 1, quantity: 1, line_total: rate, is_addon: !!a.is_addon
                  });
                });
                try {
                  ...
                  (Array.isArray(misc) ? misc : []).forEach((m) => {
                    ...
                    imported.push({
                      item_type: "custom", name: m.name || "Misc Expense",
                      description: m.notes || "", unit_rate: amt,
                      rate_type: "Fixed", days: 1, quantity: 1, line_total: amt
                    });
                  });
  ...
  const finalize = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!existingInvs || existingInvs.length === 0) { inv = await createFromQuotation(workspaceId, q, items); }
  ...
  const applyPackage = (pkg) => {
    ...
    if (items.length > 0) {
      const replace = window.confirm(`Replace current ${items.length} item(s) with ${newItems.length} item(s) from "${pkg.name}"?`);
      if (!replace) return;
    }
    setItems(newItems);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 max-w-[1100px] mx-auto">
      ...
      <QuotationPackageDialog open={showPackageDialog} onClose={() => setShowPackageDialog(false)} workspaceId={workspaceId} items={items} onApplyPackage={applyPackage} readOnly={readOnly} currency={currency} />
