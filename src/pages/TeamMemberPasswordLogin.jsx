...
// Password-only team member portal login.
// Reached via /team-login/:token — the team member enters only a password
// (no email). On success a portal session is stored and they're sent to /team-portal.
export default function TeamMemberPasswordLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await base44.functions.invoke("verifyTeamPortalAccess", { token, password });
      ...
      const msg = err?.data?.error || err?.message || "Unable to sign in. Please check your password.";
      setError(msg);
      setLoading(false);
    }
  };
