...
export default function PublicInvoice() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (d?.unavailable) { setUnavailable(true); setData(d); }
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try { const fname = `Invoice_${data?.invoice?.invoice_number || ""}`.replace(/\s+/g, "-").replace(/[^a-zA-Z0-9-_]/g, "") + ".pdf"; await generateTemplatePdf(templateHtml, { filename: fname }); }
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 py-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-card">
            ...
            {event?.start_date && <div className="text-sm text-muted-foreground mt-1">{fmtDate(event.start_date)}{event.end_date && event.end_date !== event.start_date ? ` – ${fmtDate(event.end_date)}` : ""}</div>}
            {event?.venue && <div className="text-sm text-muted-foreground mt-0.5 break-anywhere">{event.venue}</div>}
          </div>
        </div>
            ...
            <div className="p-5">
              {showItemized ? (
                    ...
                    <tbody>
                      {items.map((it, i) => (
                        <tr key={i} className="border-b border-border/50 last:border-0">
                          ...
                          <td className="py-2.5"><div className="font-medium text-foreground">{it.name}</div>{it.description && <div className="text-xs text-muted-foreground mt-0.5 break-anywhere">{it.description}</div>}</td>
                ...
                <div className="space-y-3">
                  {items.map((it, i) => (
                    <div key={i} className="flex items-start gap-3"><div className="w-1.5 h-1.5 rounded-full bg-primary mt-2 shrink-0" /><div><div className="font-medium text-foreground">{it.name}</div>{it.description && <div className="text-sm text-muted-foreground mt-0.5 break-anywhere">{it.description}</div>}</div></div>
                  ))}
                </div>
              )}
            </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-card">
          <div className="space-y-2.5">
            {showItemized && <div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span className="font-medium text-foreground tabular-nums">{formatMoney(invoice.subtotal, currency)}</span></div>}
              ...
              {bank_details.upi_id && (
                <div className="space-y-2">
                  ...
                  {(() => {
                    ...
                    return <img src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&margin=4&data=${encodeURIComponent(upiUri)}`} alt="UPI QR Code" className="w-28 h-28 rounded-lg border border-border" />;
                  })()}
