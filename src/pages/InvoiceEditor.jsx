...
export default function InvoiceEditor() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 max-w-[1000px] mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-lg p-4 space-y-3">
          <div>
            ...
            <Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} disabled={readOnly} placeholder="Auto-generated if blank" />
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={dueDateType} onChange={(e) => {
              setDueDateType(e.target.value);
              if (e.target.value === "due_on_receipt") setDueDate(invoiceDate);
              else if (e.target.value === "net_15") { const d = new Date(invoiceDate + "T00:00:00"); d.setDate(d.getDate() + 15); setDueDate(d.toISOString().slice(0, 10)); }
              else if (e.target.value === "net_30") { const d = new Date(invoiceDate + "T00:00:00"); d.setDate(d.getDate() + 30); setDueDate(d.toISOString().slice(0, 10)); }
          ...
          <div>
            ...
            <Input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} disabled={readOnly || dueDateType !== "custom"} />
          </div>
          <div>
        ...
        <div className="space-y-3">
          <div className="bg-card border border-border rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              {!readOnly && <Button type="button" variant="outline" size="sm" onClick={() => setShowClientForm(true)}><Plus className="w-3.5 h-3.5" /> New</Button>}
            </div>
      ...
      <InvoiceProductsSection items={items} setItems={setItems} readOnly={readOnly} currency={currency} />
      ...
      <InvoiceFinancials
        ...
        gstMode={gstMode} totals={totals} currency={currency} readOnly={readOnly}
      />
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        ...
        <RichTextEditor value={paymentTerms} onChange={setPaymentTerms} readOnly={readOnly} placeholder="Payment terms shown to client on PDF and public link" />
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        ...
        <WordCounterTextarea value={notes} onChange={(e) => setNotes(e.target.value)} disabled={readOnly} rows={2} placeholder="Internal notes (optional)" className="bg-card border border-border" />
      </div>
      ...
      <InvoiceBankDetailsSection bankDetails={bankDetails} setBankDetails={setBankDetails} socialLinks={socialLinks} setSocialLinks={setSocialLinks} readOnly={readOnly} />
      ...
      {!isNew && existingInvoice && (
        <div className="flex items-center justify-end gap-2 flex-wrap">
          ...
          {(status === "due" || status === "sent" || status === "partial" || status === "overdue") && (
            <Button variant="success" onClick={() => setShowPaymentDialog(true)}><Wallet className="w-4 h-4" /> Record Payment</Button>
          )}
          {status === "draft" && <Button variant="outline" onClick={sendInvoice} disabled={readOnly}>Mark as Sent</Button>}
