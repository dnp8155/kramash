...
export default function ClientPortal() {
  ...
  if (loading) {
    return (<div className="min-h-screen flex items-center justify-center bg-background"><div className="flex items-center gap-3 text-muted-foreground"><Loader2 className="w-5 h-5 animate-spin" /><span className="text-sm">Loading your portal…</span></div></div>);
  }
  if (error) {
    return (<div className="min-h-screen flex items-center justify-center bg-background p-4"><div className="max-w-md text-center space-y-3"><AlertCircle className="w-10 h-10 text-destructive mx-auto" /><h2 className="text-lg font-semibold text-foreground">Something went wrong</h2><p className="text-sm text-muted-foreground">{error}</p><button onClick={handleLogout} className="text-sm text-primary underline">Back to login</button></div></div>);
  }
  if (!data) return null;
                  ...
                  <div key={q.id} className="flex items-start gap-3 py-3">
                    ...
                    <div className="text-right shrink-0 flex flex-col items-end gap-1">
                      ...
                      {q.public_link_enabled && q.public_token && (q.status === 'finalized' || q.status === 'accepted') && (
                        <a href={`${window.location.origin}/q/${q.public_token}`} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-0.5">View <ExternalLink className="w-3 h-3" /></a>
                      )}
                    </div>
                  </div>
                );
