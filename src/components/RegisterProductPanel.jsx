      ...
      <div className="relative z-10 flex flex-col justify-between p-10 xl:p-14 text-sidebar-foreground w-full">
        ...
        <div className="max-w-md py-6">
          ...
          <div className="mt-6 grid grid-cols-2 gap-2.5">
            {categories.map((cat) => (
              <div key={cat.title} className="rounded-lg bg-sidebar-foreground/[0.06] border border-sidebar-border p-3">
              ...
              {steps.map((s) => (
                <div key={s.num} className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                    s.active ? "bg-sidebar-primary text-sidebar-primary-foreground" : "bg-sidebar-foreground/10 text-sidebar-muted border border-sidebar-border"
                  }`}>
                    {s.num}
                  </div>
                  <span className={`text-sm ${s.active ? "text-sidebar-foreground font-medium" : "text-sidebar-muted"}`}>
                    {s.label}
                  </span>
                  {s.active && <span className="ml-auto text-[10px] text-sidebar-primary font-medium">Current step</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-sidebar-muted/60">© {new Date().getFullYear()} Kramasha. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
