              ...
              {items.map((it, idx) => (
                <tr key={idx} className="border-b border-border/60">
                  <td className="py-2 pr-2">
                    <Input value={it.name} onChange={(e) => updateItem(idx, "name", e.target.value)} disabled={readOnly} className={cn("h-8", itemErrors[idx]?.name && "border-destructive bg-destructive/5")} placeholder="Item name" />
                    <input value={it.description || ""} onChange={(e) => updateItem(idx, "description", e.target.value)} disabled={readOnly} placeholder="Description" className="w-full text-xs text-muted-foreground bg-transparent border-0 focus:outline-none mt-0.5 placeholder:text-muted-foreground/60" />
                  </td>
