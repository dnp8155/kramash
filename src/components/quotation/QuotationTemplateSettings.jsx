    ...
    <Section title="Template Settings">
      ...
      <div className="pt-2 border-t border-border space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Switch checked={cfg.bank?.enabled !== false} onCheckedChange={(v) => updateSection("bank.enabled", v)} disabled={readOnly} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Bank Name"><Input value={cfg.bank?.bankName || ""} onChange={(e) => updateSection("bank.bankName", e.target.value)} disabled={readOnly} /></Field>
          <Field label="Account Name"><Input value={cfg.bank?.accountName || ""} onChange={(e) => updateSection("bank.accountName", e.target.value)} disabled={readOnly} /></Field>
          <Field label="Account Number"><Input value={cfg.bank?.accountNumber || ""} onChange={(e) => updateSection("bank.accountNumber", e.target.value)} disabled={readOnly} /></Field>
          <Field label="IFSC Code"><Input value={cfg.bank?.ifsc || ""} onChange={(e) => updateSection("bank.ifsc", e.target.value)} disabled={readOnly} /></Field>
          <Field label="Branch"><Input value={cfg.bank?.branch || ""} onChange={(e) => updateSection("bank.branch", e.target.value)} disabled={readOnly} /></Field>
        </div>
      </div>
