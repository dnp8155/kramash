    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border">
        ...
        <div className="p-5 sm:p-6">
          ...
          {client?.address && <div className="text-sm text-muted-foreground mt-1">{client.address}</div>}
          {q.context_type && (
      ...
      {(event?.title || event?.venue) && (
        <div className="px-5 sm:px-6 py-4 border-t border-border bg-muted/30">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-sm">
            ...
            {event?.venue && (
              <span className="inline-flex items-center gap-1.5 text-muted-foreground"><MapPin className="w-3.5 h-3.5 shrink-0" /> {event.venue}</span>
            )}
          </div>
        </div>
      )}
