...
export default function ProfileWorkspaceSection() {
  ...
  if (!form && workspace) {
    const initial = {
      ...
      timezone: workspace.timezone || "Asia/Kolkata", date_format: workspace.date_format || "DD/MM/YYYY",
  ...
  const saveWorkspace = async () => {
    if (form.phone && !isValidIndianMobile(form.phone)) {
      toast({ title: "Invalid phone", description: "Enter a valid 10-digit Indian mobile number (starts with 6-9).", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const updatedDp = {
        ...
        social_website: form.social_website, social_portfolio: form.social_portfolio,
      };
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        ...
        <div className="space-y-3">
          ...
          <Field label="Business Phone">
            <div className="relative">
              <Input value={form.phone} onChange={(e) => set("phone", sanitizePhoneInput(e.target.value))} readOnly={isPhoneLocked} placeholder="10-digit mobile (e.g. 9876543210)" inputMode="tel" maxLength={10} className={isPhoneLocked ? "bg-muted/50 cursor-not-allowed text-muted-foreground pr-8" : "pr-8"} />
          ...
          <Field label="Business Email">
            <div className="relative">
              <Input value={form.email} onChange={(e) => set("email", e.target.value)} readOnly={isEmailLocked} placeholder="you@example.com" className={isEmailLocked ? "bg-muted/50 cursor-not-allowed text-muted-foreground pr-8" : "pr-8"} />
              ...
              <Field label="Profile URL Slug">
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative flex-1">
                    <Input value={form.public_profile_slug} onChange={(e) => set("public_profile_slug", slugify(e.target.value))} placeholder="your-business" readOnly={isSlugLocked} className={isSlugLocked ? "w-full bg-muted/50 cursor-not-allowed text-muted-foreground pr-8" : "w-full pr-8"} />
                ...
                {form.public_profile_slug && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/p/${form.public_profile_slug}`); setCopiedSlug(true); setTimeout(() => setCopiedSlug(false), 2000); }} className="shrink-0 p-2 rounded-md border border-border hover:bg-muted transition-colors" aria-label="Copy link">
