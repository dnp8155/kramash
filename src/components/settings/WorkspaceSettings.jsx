      ...
      <div className="bg-card border border-border rounded-lg p-4">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-14 h-14 rounded-full bg-muted border border-border flex items-center justify-center overflow-hidden">
            {form.logo ? <img src={form.logo} alt="Workspace logo" className="w-full h-full object-cover" /> : <Upload className="w-5 h-5 text-muted-foreground" />}
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Business Phone"><Input value={form.phone} onChange={(e) => set("phone", sanitizePhoneInput(e.target.value))} placeholder="10-digit mobile (e.g. 9876543210)" inputMode="tel" maxLength={10} /></Field>
            <Field label="Business Email">
              <div className="flex items-center gap-2">
                <Input value={form.email} onChange={(e) => set("email", e.target.value)} readOnly={!emailEditing} placeholder="you@example.com" className={emailEditing ? "" : "bg-muted/50 cursor-not-allowed text-muted-foreground"} />
                {emailEditing ? (
                  <button type="button" onClick={() => setEmailEditing(false)} className="shrink-0 w-9 h-9 rounded-md border border-border flex items-center justify-center text-success hover:bg-success/10 transition-colors" aria-label="Confirm email edit" title="Done">
                    <Check className="w-4 h-4" />
                  </button>
                ) : (
                  <button type="button" onClick={() => setEmailEditing(true)} className="shrink-0 w-9 h-9 rounded-md border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" aria-label="Edit email" title="Edit email">
