  ...
  const confirmChange = async () => {
    ...
    try {
      ...
      const updated = await base44.entities.Workspace.update(workspace.id, {
        ...
        custom_work_label_singular: "",
        custom_work_label_plural: "",
      });

      // 2. Delete + reseed TeamRoles
      try {
        ...
        if (newProfile.roles.length > 0) {
          await base44.entities.TeamRole.bulkCreate(
            newProfile.roles.map((r) => ({ ...r, workspace_id: workspace.id, status: "active" }))
          );
        }
      } catch (e) { /* non-fatal */ }

      // 3. Delete + reseed Services
      try {
        ...
        if (newProfile.services.length > 0) {
          await base44.entities.Service.bulkCreate(
            newProfile.services.map((s) => ({ ...s, workspace_id: workspace.id, status: "active" }))
          );
        }
      } catch (e) { /* non-fatal */ }
      ...
      <AlertDialog open={!!confirmCategory} onOpenChange={(open) => { if (!open) setConfirmCategory(null); }}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={(e) => { e.preventDefault(); confirmChange(); }}
              disabled={saving}
