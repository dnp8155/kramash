...
export default function QuickClientForm({ workspaceId, onSaved, onCancel }) {
  ...
  const submit = async () => {
    ...
    if (phone && !isValidIndianMobile(phone)) { setError("Enter a valid 10-digit Indian mobile number (starts with 6-9)."); return; }
    ...
    try {
      ...
      onSaved?.(created);
    } catch (e) { setError(e?.message || "Failed to add client."); }
    finally { stop(); }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <Input value={phone} onChange={(e) => setPhone(sanitizePhoneInput(e.target.value))} placeholder="10-digit mobile" inputMode="tel" maxLength={10} disabled={saving} />
