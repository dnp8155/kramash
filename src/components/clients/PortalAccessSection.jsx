...
const PortalAccessSection = forwardRef(function PortalAccessSection({ client, workspaceId }, ref) {
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      reload();
      toast({ title: "Portal access enabled", description: "Share the link and password with your client." });
    } catch (e) {
  ...
  const handleDisable = async () => {
    if (!confirm("Disable portal access for this client? They will no longer be able to sign in with their password.")) return;
    setBusy(true);
    try {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      savePw(client.id, d.password || "");
      setShowPassword(true);
      toast({ title: "New password generated", description: "Share the new password with your client." });
    } catch (e) {
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-foreground">Quick Portal Access (password-only)</div>
            <p className="text-xs text-muted-foreground mt-0.5 mb-3">
              Give this client a simple link + password to view their portal — no email login needed.
            </p>
    ...
    <div className="bg-card border border-border rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-semibold text-foreground">Quick Portal Access</div>
      ...
      <div>
        ...
        <p className="text-[11px] text-muted-foreground mt-1.5">
          Click the eye icon to show/hide the password. Click copy to share it.
        </p>
      ...
      {/* Quick Share */}
      {(loginUrl || client.portal_access_token) && password && (
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => {
                ...
                const phone = client.phone ? `91${client.phone.replace(/\D/g, "").replace(/^91/, "")}` : "";
                const msg = `Hi ${client.name}, here is your portal link:\n${link}\n\nPassword: ${password}`;
                const waUrl = phone
