    ...
    <AppDialog open={open} onOpenChange={(o) => !o && onClose?.()}>
      <AppDialogContent>
        <AppDialogHeader>
          ...
          <AppDialogDescription>Add a team member to this event.</AppDialogDescription>
        </AppDialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <AppDialogBody className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              {members.length === 0 && <p className="text-xs text-muted-foreground">No team members available. Add members from the Team page.</p>}
              {selfAlreadyAssigned && <p className="text-xs text-muted-foreground">Owner (Self) is already assigned to this event and cannot be added again.</p>}
              {alreadyAssigned && <p className="text-xs text-destructive font-medium">{selectedMember?.name} is already assigned to this event.</p>}
            ...
            <div className="space-y-1.5">
              ...
              {workingDates.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  ...
                  {rateType === "Per Day" && roleRateConfigured ? ` · Calculated: ${formatMoney(Number(roleRate) * workingDates.length, currency)}` : ""}
                </p>
              )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" min="0" step="0.01" value={agreedRate} onChange={(e) => setAgreedRate(e.target.value)} placeholder="0" />
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Toggle checked={recordPaymentNow && !selectedMemberIsSelf} onChange={selectedMemberIsSelf ? () => {} : setRecordPaymentNow} label="Record Payment" />
              </div>
              {selectedMemberIsSelf ? (
                <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-3 pt-1 border-t border-border">
                  <p className="text-xs text-muted-foreground">Creates a team payment transaction. Financial Year is auto-assigned from the payment date.</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <Label>Payment Amount (₹) <span className="text-destructive">*</span></Label>
                      <Input type="number" min="0.01" step="0.01" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} placeholder="0" />
                    </div>
                    <div className="space-y-1.5">
                  ...
                  {paymentDate && (() => {
                    ...
                    return fy ? (
                      ...
                      <p className="text-xs text-destructive">No Financial Year covers this date. Create the applicable FY first.</p>
                    );
                  })()}
                </div>
              )}
                ...
                <div className="flex items-start gap-2 text-amber-800">
                  ...
                  <div className="text-sm">
                    ...
                    <ul className="mt-1 list-disc list-inside text-xs">
                      {conflicts.map((c) => <li key={c.id}>{c.title} · {formatEventDate(c.start_date, c.end_date)}</li>)}
                    </ul>
                  </div>
                </div>
            ...
            {blockConflicts.length > 0 && (
              <div className="rounded-md border border-slate-300 bg-slate-50 p-3 space-y-2">
                <div className="flex items-start gap-2 text-slate-700">
                  ...
                  <div className="text-sm">
                    <div className="font-semibold">{selectedMember?.name} has blocked dates overlapping this event:</div>
                    <ul className="mt-1 list-disc list-inside text-xs">
