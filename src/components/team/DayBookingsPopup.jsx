        ...
        <div className="p-5 space-y-4">
          {bookings.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                <span className="status-dot w-2 h-2 rounded-full bg-[#e74c3c]" /> Booked ({bookings.length})
              </div>
                  ...
                  <div key={`${member.id}-${ev?.id}`} className="border border-border rounded-lg p-3 bg-muted/30">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">{member.profession || assignment?.role_name_snapshot || "Team Member"}</div>
                      </div>
                    </div>
                    {ev && (
                      <button
                        ...
                        <div className="mt-2 grid grid-cols-1 gap-1.5 text-xs">
                          {ev.venue && (
                            <div className="flex items-start gap-1.5 text-muted-foreground">
                              ...
                              <span className="break-words">{ev.venue}{ev.venue_address ? `, ${ev.venue_address}` : ""}</span>
                            </div>
                          )}
          ...
          {blocks.length > 0 && (
            <div className={bookings.length > 0 ? "pt-3 border-t border-border" : ""}>
              <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                <span className="status-dot w-2 h-2 rounded-full bg-[#6b7280]" /> Blocked / Leave ({blocks.length})
              </div>
