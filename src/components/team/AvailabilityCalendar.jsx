...
export default function AvailabilityCalendar({
  ...
  const [weekRef, setWeekRef] = useState(() => toISODate(new Date()));
  const [dayRef, setDayRef] = useState(() => toISODate(new Date()));
  ...
  const selectedInfo = useMemo(() => {
    if (!selected) return { available: [], blocked: [] };
  ...
  const upcomingEvents = useMemo(() => {
    ...
    for (const ev of allEvents) {
      const dates = Array.isArray(ev.event_dates) && ev.event_dates.length > 0
        ...
        : (ev.start_date ? [ev.start_date] : []);
      for (const d of dates) {
        if (d >= today) rows.push({ event: ev, date: d });
      }
    }
      ...
      <div className="bg-card border border-border rounded-xl p-5 shadow-card">
        <div className="flex items-center justify-between gap-2 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            {viewMode === "month" && (
              <>
                <button onClick={prevYear} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Previous year"><ChevronsLeft className="w-4 h-4" /></button>
                <button onClick={prevMonth} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Previous month"><ChevronLeft className="w-4 h-4" /></button>
                <h3 className="text-base font-bold text-foreground px-2 min-w-[140px] text-center">{MONTHS[view.m]} {view.y}</h3>
                <button onClick={nextMonth} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Next month"><ChevronRight className="w-4 h-4" /></button>
                <button onClick={nextYear} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Next year"><ChevronsRight className="w-4 h-4" /></button>
              </>
            )}
            {viewMode === "year" && (
              <>
            ...
            {viewMode === "week" && (
              <>
                <button onClick={prevWeek} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Previous week"><ChevronLeft className="w-4 h-4" /></button>
                ...
                <button onClick={nextWeek} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Next week"><ChevronRight className="w-4 h-4" /></button>
              </>
            )}
            {viewMode === "day" && (
              <>
                <button onClick={prevDay} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Previous day"><ChevronLeft className="w-4 h-4" /></button>
                ...
                <button onClick={nextDay} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground transition-colors" aria-label="Next day"><ChevronRight className="w-4 h-4" /></button>
              </>
            )}
          </div>
        </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isSelected ? "border-primary bg-primary/5 shadow-sm"
                      : hasMultiple ? "border-[#f39c12] bg-[#f39c12]/5 hover:border-[#f39c12]"
                      : hasBooked ? "border-[#e74c3c] bg-[#e74c3c]/5 hover:border-[#e74c3c]"
                      : hasBlocked ? "border-[#6b7280] bg-[#6b7280]/5 hover:border-[#6b7280]"
                  ...
                  {hasBooked && (
                    <div className="flex flex-col gap-0.5 w-full overflow-hidden mt-0.5">
                      {dayEvents.slice(0, 2).map((ev) => (
                        <span key={ev.id} title={ev.title} className={cn(
                      ...
                      {dayEvents.length > 2 && <span className="text-[9px] font-semibold text-[#d97706] leading-tight px-1">+{dayEvents.length - 2} more</span>}
                    </div>
                  )}
                  {!hasBooked && hasBlocked && (
                    <div className="flex flex-col gap-0.5 w-full overflow-hidden mt-0.5">
                      {blocked.slice(0, 1).map((b) => (
                        <span key={b.member.id} title={`${b.member.name} — ${b.block?.reason || "Blocked"}`} className="text-[9px] font-medium px-1 py-0.5 rounded leading-tight truncate bg-[#6b7280]/15 text-[#6b7280] flex items-center gap-0.5">
                          <Ban className="w-2 h-2 shrink-0" />{b.member.name.split(" ")[0]}
                        </span>
                      ))}
                      {blocked.length > 1 && <span className="text-[9px] font-semibold text-[#6b7280] leading-tight px-1">+{blocked.length - 1}</span>}
                    </div>
                  )}
                </button>
              );
                ...
                >
                  ...
                  {hasBooked && (
                    <div className="flex flex-col gap-0.5 w-full overflow-hidden mt-1">
                      ...
                      {dayEvents.length > 4 && <span className="text-[9px] font-semibold text-[#d97706] leading-tight px-1">+{dayEvents.length - 4} more</span>}
                    </div>
                  )}
                  {!hasBooked && hasBlocked && (
                    <div className="flex flex-col gap-0.5 w-full overflow-hidden mt-1">
                      ...
                      {blocked.length > 3 && <span className="text-[9px] font-semibold text-[#6b7280] leading-tight px-1">+{blocked.length - 3}</span>}
                    </div>
                  )}
                </button>
              );
            ...
            <div>
              ...
              <div className="space-y-3">
                {dayEvents.length > 0 && (
                  <div>
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">
                      <span className="status-dot w-2 h-2 rounded-full bg-[#e74c3c]" /> Events ({dayEvents.length})
                    </div>
                    <div className="space-y-1.5">
                      {dayEvents.map((ev) => (
                        <button key={ev.id} onClick={() => onEventClick?.(ev)} className="w-full text-left text-sm px-3 py-2 rounded-lg border border-[#e74c3c]/20 bg-[#e74c3c]/5 hover:bg-[#e74c3c]/10 transition-colors truncate">
                          {ev.title}
                        </button>
                      ))}
                    </div>
                ...
                {dayInfo.available.length > 0 && (
                  <div>
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">
                      <span className="status-dot w-2 h-2 rounded-full bg-[#27ae60]" /> Available ({dayInfo.available.length})
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {dayInfo.available.map((m) => (
                        <span key={m.id} className="text-xs px-2 py-1 rounded-full bg-[#27ae60]/10 text-[#27ae60] border border-[#27ae60]/20 inline-flex items-center gap-1">
                          {m.name}
                          {m.is_self && <span className="inline-flex items-center gap-0.5 px-1 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide bg-primary text-primary-foreground"><Crown className="w-2 h-2" /> Self</span>}
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                {dayInfo.blocked.length > 0 && (
                  <div>
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">
                      <span className="status-dot w-2 h-2 rounded-full bg-[#6b7280]" /> Blocked ({dayInfo.blocked.length})
                    </div>
                    <div className="space-y-1.5">
                      {dayInfo.blocked.map(({ member, block }) => (
                        <div key={member.id} className="flex items-center justify-between gap-2 text-sm px-3 py-1.5 rounded-lg bg-[#6b7280]/5 border border-[#6b7280]/15">
                          <span className="min-w-0 flex items-center gap-1.5">
                            ...
                            {member.is_self && <span className="inline-flex items-center gap-0.5 px-1 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide bg-primary text-primary-foreground shrink-0"><Crown className="w-2 h-2" /> Self</span>}
                            {block?.reason ? <span className="text-muted-foreground"> — {block.reason}</span> : null}
                          </span>
                          {onUnblockDate && block?.id && (
                            <button onClick={() => onUnblockDate(block.id)} className="text-xs font-medium text-success hover:underline shrink-0 flex items-center gap-0.5">
        ...
        <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
          <span className="flex items-center gap-1.5"><span className="status-dot w-2.5 h-2.5 rounded-full bg-[#27ae60]" /> Available</span>
          <span className="flex items-center gap-1.5"><span className="status-dot w-2.5 h-2.5 rounded-full bg-[#e74c3c]" /> Booked</span>
          <span className="flex items-center gap-1.5"><span className="status-dot w-2.5 h-2.5 rounded-full bg-[#f39c12]" /> Multiple Bookings</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#6b7280]" /> Blocked / Leave</span>
        </div>
      </div>
        ...
        {!selected ? (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              {upcomingEvents.length > 0 && <span className="text-xs text-muted-foreground ml-auto">{upcomingEvents.length}</span>}
            </div>
            {upcomingEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-10">
                ...
                <p className="text-xs text-muted-foreground mt-1 max-w-[200px]">No upcoming events. Pick a day on the calendar to see details.</p>
              </div>
            ) : (
              <div className="space-y-2 max-h-[70vh] overflow-y-auto -mr-1 pr-1">
                {upcomingEvents.map(({ event: ev, date }) => (
                  <button key={`${ev.id}-${date}`} onClick={() => onEventClick?.(ev)} className="w-full flex items-center gap-3 p-2.5 rounded-lg border border-border hover:border-primary/30 hover:bg-muted/50 transition-colors text-left">
                    <div className="flex flex-col items-center justify-center w-11 h-11 rounded-lg bg-primary/10 text-primary shrink-0">
                      <span className="text-[10px] font-semibold uppercase leading-none">{new Date(date + "T12:00:00").toLocaleDateString("en-IN", { month: "short" })}</span>
                      <span className="text-lg font-bold leading-none mt-0.5">{Number(date.slice(8))}</span>
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium text-foreground truncate">{ev.title}</div>
                      <div className="text-xs text-muted-foreground truncate mt-0.5">{ev.event_type || "Event"} {ev.venue ? `· ${ev.venue}` : ""}</div>
                    </div>
                    {ev.status === "upcoming" && <span className="status-dot w-2 h-2 rounded-full bg-[#27ae60] shrink-0" title="Upcoming" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {parseISODate(selected) ? parseISODate(selected).toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" }) : selected}
              </div>
              {onBlockDate && (
                <button onClick={() => onBlockDate(selected)} className="text-xs font-medium text-warning hover:underline flex items-center gap-1">
            ...
            {selectedBlocked.length > 0 && (
              <div className="mt-3 pt-3 border-t border-border">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">
                  <span className="status-dot w-2 h-2 rounded-full bg-[#6b7280]" /> Blocked ({selectedBlocked.length})
                </div>
            ...
            <div className="mt-3 space-y-3">
              <div>
                <div className="flex items-center gap-1.5 text-sm font-semibold mb-1.5">
                  <span className="status-dot w-2 h-2 rounded-full bg-[#27ae60]" /> Available ({selectedInfo.available.length})
                </div>
              ...
              {selectedInfo.blocked.length > 0 && (
                <div className="pt-3 border-t border-border">
                  <div className="flex items-center gap-1.5 text-sm font-semibold mb-1.5">
                    <span className="status-dot w-2 h-2 rounded-full bg-[#6b7280]" /> Blocked ({selectedInfo.blocked.length})
                  </div>
                  <ul className="space-y-1.5 pl-3.5">
                    {selectedInfo.blocked.map(({ member, block }) => (
                      <li key={member.id} className="text-sm flex items-center justify-between gap-2">
                        ...
                        {onUnblockDate && block?.id && (
                          <button onClick={() => onUnblockDate(block.id)} className="text-xs font-medium text-success hover:underline shrink-0 flex items-center gap-0.5" title="Unblock this member">
                            <Unlock className="w-3 h-3" /> Unblock
                          </button>
                        )}
                      </li>
              ...
              {allActiveBlocks.map(({ member, block }) => {
                ...
                return (
                  <li key={block.id} className="text-sm flex items-center justify-between gap-2 p-1.5 rounded-md hover:bg-muted/50">
                    ...
                    {onUnblockDate && block.id && (
                      <button onClick={() => onUnblockDate(block.id)} className="text-xs font-medium text-success hover:underline shrink-0 flex items-center gap-0.5 px-2 py-1 rounded-md hover:bg-success/10">
                        <Unlock className="w-3 h-3" /> Unblock
                      </button>
                    )}
                  </li>
