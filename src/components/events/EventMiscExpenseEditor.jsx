...
// Parses the misc_expenses_json string into an array. Returns [] on any error.
...
// Sums the amounts of a misc expenses array.
export function miscExpensesTotal(items) {
  return (items || []).reduce((s, x) => s + (Number(x.amount) || 0), 0);
}

// Editor for event-level misc expenses — each item has a name, amount, and optional notes.
// These add to the event's effective contract value (billable to the client).
