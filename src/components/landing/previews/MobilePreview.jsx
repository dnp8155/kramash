    ...
    <div className="w-[200px] rounded-[2rem] border-[3px] border-[#1A1A1A] bg-[#0A0A0A] p-1.5 shadow-2xl shrink-0">
      <div className="relative">
        ...
        <div className="rounded-[1.7rem] bg-[#F5F3EF] overflow-hidden h-[380px] relative">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-[#E8E3DB] flex justify-around py-2">
            {[LayoutDashboard, CalendarCheck, Wallet, Users].map((Icon, i) => (
              <Icon key={i} className={`w-4 h-4 ${i === 0 ? "text-[#C8A95E]" : "text-[#8A8580]"}`} />
            ))}
          </div>
        </div>
      </div>
