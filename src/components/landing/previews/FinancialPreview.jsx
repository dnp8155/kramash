...
export default function FinancialPreview() {
  const stats = [
    { label: "Total Received", value: "₹4,82,450", icon: Wallet, tone: "primary" },
  ...
  const toneColor = (tone) =>
    tone === "primary" ? "text-[#F58220]" : tone === "warning" ? "text-[#E0741F]" : tone === "danger" ? "text-[#DC2626]" : "text-[#F58220]";
  ...
  return (
    <div className="rounded-2xl border border-[#E5E5E5] bg-white shadow-xl overflow-hidden">
      ...
      <div className="px-5 py-3.5 border-b border-[#E5E5E5]">
        ...
        <div className="text-[10px] text-[#999]">Financial Year 2026-27</div>
      </div>
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          {stats.map((s, i) => (
            <div key={i} className="rounded-lg border border-[#E5E5E5] bg-[#F9F9F9] p-3">
              <div className="flex items-center justify-between mb-1.5">
                <div className="text-[10px] text-[#999] uppercase tracking-wide">{s.label}</div>
                <s.icon className={`w-3.5 h-3.5 ${toneColor(s.tone)}`} />
              </div>
        ...
        {/* Mini bar chart */}
        <div className="rounded-lg border border-[#E5E5E5] bg-[#F9F9F9] p-3 mb-4">
          <div className="text-[10px] font-semibold text-[#999] uppercase tracking-wide mb-2">Monthly Revenue</div>
          <div className="flex items-end gap-1.5 h-16">
            {[40, 55, 45, 70, 60, 85, 75, 90, 65, 80, 95, 100].map((h, i) => (
              <div
                key={i}
                className="flex-1 rounded-t bg-gradient-to-t from-[#F58220]/40 to-[#F58220]"
                style={{ height: `${h}%` }}
              />
            ))}
          </div>
        ...
        <div className="rounded-lg border border-[#E5E5E5] overflow-hidden">
          <div className="px-3 py-2 bg-[#F9F9F9] text-[10px] font-semibold text-[#999] uppercase tracking-wide">
            Recent Transactions
          </div>
          {transactions.map((t, i) => (
            <div key={i} className="flex items-center justify-between px-3 py-2 border-t border-[#E5E5E5]">
              <div className="min-w-0">
                <div className="text-[11px] font-medium text-foreground truncate">{t.desc}</div>
                <div className="text-[9px] text-[#999]">{t.method}</div>
              </div>
              <span className={`text-xs font-semibold shrink-0 ml-2 ${
                t.type === "in" ? "text-[#F58220]" : "text-[#DC2626]"
              }`}>
                {t.amount}
              </span>
            </div>
