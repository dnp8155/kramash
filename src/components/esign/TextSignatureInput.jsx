...
export default function TextSignatureInput({ onChange, color = "#000000" }) {
  ...
  const render = useCallback(async (value) => {
    if (!value.trim() || !fontReady) { onChange(""); return; }
    const fontSize = 56;
  ...
  return (
    <div>
      <div className="relative border border-border rounded-lg bg-card overflow-hidden h-36 flex items-center justify-center">
        {text.trim() && fontReady ? (
          <span className="text-5xl leading-none" style={{ fontFamily: '"Caveat", cursive', color }}>{text}</span>
        ) : (
      ...
      <div className="mt-2">
        <Input type="text" value={text} onChange={(e) => setText(e.target.value)} placeholder="e.g. John Doe, JD, 16/09/2026" maxLength={60} className="w-full" />
      </div>
    </div>
  );
}