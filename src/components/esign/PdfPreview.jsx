...
// parent can switch to the simple page+corner picker.
export default function PdfPreview({
  ...
  const [dragMode, setDragMode] = useState(null); // "drag" | "resize" | null
  const dragStart = useRef(null);

  // Render the requested page whenever file or page changes
  useEffect(() => {
    if (!file) return;
    let cancelled = false;
    setLoading(true);
  ...
  useEffect(() => {
    ...
    const onMove = (ev) => {
      ...
      if (dragMode === "drag") {
        onOverlayChange({
          ...overlay,
          x: Math.max(0, Math.min(s.ox + dx, renderedSize.width - s.ow)),
          y: Math.max(0, Math.min(s.oy + dy, renderedSize.height - s.oh)),
        });
      } else {
        onOverlayChange({
          ...overlay,
          width: Math.max(60, Math.min(s.ow + dx, renderedSize.width - overlay.x)),
          height: Math.max(30, Math.min(s.oh + dy, renderedSize.height - overlay.y)),
        });
      }
    };
    ...
    <div ref={containerRef} className="relative w-full flex justify-center bg-muted/20 border border-border rounded-lg overflow-hidden p-2">
      ...
      {renderedSize && (
        <div
          ...
          style={{
            left: `calc(50% - ${renderedSize.width / 2}px + ${overlay.x}px)`,
            top: `${overlay.y + 8}px`,
        ...
        >
          ...
          <div
            className="absolute -bottom-1.5 -right-1.5 w-4 h-4 bg-primary rounded-sm cursor-nwse-resize touch-none flex items-center justify-center"
