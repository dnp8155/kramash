...
export function renderInvoiceGoldPremium(data) {
  ...
  const clientAddressLines = [cli.address, [cli.city, cli.state].filter(Boolean).join(", "), cli.country].filter(Boolean);
  ...
  const packageSections = packages.map((pkg) => {
    ...
    return `
      <div class="package-block">
        ...
        <table>
          <thead>
            <tr><th>#</th><th>EVENT</th><th>DATE</th><th>LOCATION</th><th>SLOT</th></tr>
          </thead>
          <tbody>
${eventRows}
          </tbody>
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root { --gold: #d99a00; --gold-dark: #b97d00; --black: #151515; --soft: #f7f7f7; --border: #dddddd; --muted: #666; --danger: #c0392b; }
    .invoice { width: 1120px; max-width: 100%; margin: auto; background: #ffffff; padding: 42px 42px 0; }
...
${lineItems.length > 0 ? `
    ...
    <section class="table-wrap">
      <table>
        <thead>
          <tr><th>#</th><th>DESCRIPTION</th><th>DETAILS</th><th>QTY</th><th>AMOUNT (${escapeHtml(currency || "INR")})</th></tr>
        </thead>
        <tbody>
${lineItemRows}
        </tbody>
    ...
    <section class="bottom-section">
      <div>
        ...
        <div class="info-block">
          <div class="info-heading"><div class="round-icon">&#9635;</div><span>NOTES</span></div>
          <div class="editable-list">${notesHtml}</div>
        </div>` : ""}
        ${termsHtml ? `
        <div class="info-block">
          <div class="info-heading"><div class="round-icon">&#10003;</div><span>TERMS &amp; CONDITIONS</span></div>
          <div class="editable-list">${termsHtml}</div>
        </div>` : ""}
      </div>
      <div>
        <div class="total-card">
          <div class="total-row"><span>SUBTOTAL</span><span>${money(subtotal, symbol)}</span></div>
          ${discountAmount > 0 ? `<div class="total-row"><span>${discountLabel}</span><span class="discount-value">- ${money(discountAmount, symbol)}</span></div>` : ""}
          ${gstApplicable && gstTotal > 0 ? `<div class="total-row"><span>TAX (${gstRate}% GST)</span><span>${money(gstTotal, symbol)}</span></div>` : ""}
          <div class="total-row grand-total"><span>TOTAL AMOUNT</span><span>${money(grandTotal, symbol)}</span></div>
        </div>
      </div>
    </section>
