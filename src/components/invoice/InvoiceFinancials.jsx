...
export default function InvoiceFinancials({
  ...
  gstRate, setGstRate,
  gstMode, totals, currency, readOnly
}) {
      ...
      <div className="bg-card border border-border rounded-lg p-4 space-y-4">
        ...
        <div className="border-t border-border pt-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={!!gstApplicable} onChange={(v) => !readOnly && setGstApplicable(v)} disabled={readOnly} />
          </div>
          {gstApplicable && (
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Select
                  ...
                  onChange={(e) => { if (e.target.value === "custom") return; setGstRate(Number(e.target.value)); }}
                  disabled={readOnly}
                  className="flex-1"
                >
                  {gstRates.map((r) => <option key={r} value={String(r)}>{r}%</option>)}
                  {!gstRates.includes(Number(gstRate)) && Number(gstRate) > 0 && <option value="custom">{gstRate}% (custom)</option>}
                </Select>
                <Input type="number" value={gstRate || 0} onChange={(e) => setGstRate(e.target.value)} disabled={readOnly} className="w-20" min="0" max="100" />
              </div>
