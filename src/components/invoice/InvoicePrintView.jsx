    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex flex-col">
      <div className="flex items-center justify-between gap-2 px-4 py-3 bg-card border-b border-border">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="outline" onClick={handlePrint} disabled={downloading} style={{ borderColor: "#CCCCCC", backgroundColor: "#FFFFFF", color: "#000000" }}>
            <Download className="w-4 h-4" /> <span className="hidden sm:inline">{downloading ? "…" : "PDF"}</span>
          </Button>
          {onEdit && (
