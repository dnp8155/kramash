...
import { LayoutGrid, CalendarDays, Users, UserCircle, Wallet, FileText } from "lucide-react";
      ...
      <div className="relative w-[112%] -mr-[12%] rounded-2xl shadow-2xl shadow-slate-400/15 border border-slate-200/50 overflow-hidden bg-card">
        <div className="flex h-[440px] xl:h-[470px]">
          <div className="w-12 bg-sidebar flex flex-col items-center py-4 gap-2.5">
            ...
            {navItems.map((item, i) => (
              <div key={i} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${item.active ? "bg-sidebar-primary/20" : ""}`}>
                <item.icon className={`w-[15px] h-[15px] ${item.active ? "text-sidebar-primary" : "text-sidebar-muted"}`} />
              </div>
            ))}
          ...
          <div className="flex-1 bg-background p-5 overflow-hidden">
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <div className="flex items-center -space-x-1.5">
                {teamAvatars.map((t, i) => (
                  <div key={i} className={`w-6 h-6 rounded-full bg-gradient-to-br ${t.gradient} ring-2 ring-background flex items-center justify-center text-[8px] font-bold text-white`}>
                    {t.initials}
                  </div>
                ))}
