...
// Uses the client-side createFromQuotation for race-condition protection.
export async function createInvoiceFromQuotation(workspaceId, quotationId, mode, options = {}) {
  ...
  if (!quotation || quotation.workspace_id !== workspaceId) throw new Error("Quotation not found");
  const items = await base44.entities.QuotationItem.filter({ workspace_id: workspaceId, quotation_id: quotationId }, "sort_order", 500);
  return createFromQuotation(workspaceId, quotation, items);
}
