...
export function eventFinancialSummary(event, transactions, assignments, serviceAssignments) {
  ...
  // Add-on total from service assignments
...
// ---- Client-side transaction wrappers (void / edit / delete with reconciliation) ----
// Replaces the Edge Functions that are not deployed on Supabase.

import { voidTransaction as voidTxn, deleteTransactionFn, editTransaction as editTxn } from "@/lib/clientEdgeFunctions";

export async function voidTransaction(workspaceId, transactionId) {
  return voidTxn(workspaceId, transactionId);
}

export async function deleteTransaction(workspaceId, transactionId) {
  return deleteTransactionFn(workspaceId, transactionId);
}

export async function editTransaction(workspaceId, transactionId, changes) {
  return editTxn(workspaceId, transactionId, changes);
}
