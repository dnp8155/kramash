...
// Used by both getClientPortalData (invited-user path) and
// getClientPortalDataByAccess (password-only portal path) so the two
...
export async function buildClientPortalData(base44, clientId, workspaceId, options = {}) {
  ...
  try {
    clientRecord = await base44.asServiceRole.entities.Client.get(clientId);
  } catch (e) { /* continue */ }
